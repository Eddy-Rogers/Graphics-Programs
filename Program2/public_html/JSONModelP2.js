/**
 * JSONModel  (ver. 2.2) - draw model loaded from a JSON file
 * 
 ...
 * 
 * Version 2.2 - add bounding box computation in modeling coord on initialization
 * 
...
function JSONModel(gl, modelURL) {
  ...
  // Also find the min and max extent along each axis.
  this.faceVertexCount = [];
  var vertexCoords = [];
  var vertexColors = [];
  
  // minBounds has the minimum x, y, z values of any point
  // maxBounds has the maximum x, y, z values of any point
  this.minBounds = [ Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY ];
  this.maxBounds = [ Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY ];
  ...
  for (var i = 0; i < this.numFaces; ++i) {
    ...
    for (var j = 0; j < numVertices; ++j) {
      ...
      // Update bounding box
      for (var d = 0; d < 3; ++d) {
        this.minBounds[d] = Math.min(this.minBounds[d], face.vertexCoords[j][d]);
        this.maxBounds[d] = Math.max(this.maxBounds[d], face.vertexCoords[j][d]);
      } 
    }
  }
  this.ySize = this.maxBounds[1] - this.minBounds[1];
